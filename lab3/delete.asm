...
Delete_x:
	...
	lw	$t2, -4($s0)	#load address of previous node
	beqz	$t2, $a0	#if no previous node, this is a head node
	
	lw	$t3, 12($s1)	#load address of next node
	beqz	$t3, $a0	#if no previous node, this is a tail node
	
	lw	$t3, 12($s1)	#load address of next node
	sw	$t2, -4($t3)	#store address of previous node in next node's previous field
	
	lw	$t2, 12($s0)	# load address of next node
	lw	$t3, -4($s1)	# load address of previous node 
	sw	$t2, 12($t3)	#store address of next node in previous node's next field
	
	la	$s0, ($t2)	#the new curr is the next node
